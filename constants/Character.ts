import { Letter } from "./Letters";
import { NumberChar } from "./Numbers";

export type Character = NumberChar | Letter;

export const isLetter = (char: Character): char is Letter => {
  return Object.values(Letter).includes(char as Letter);
};

export const isNumber = (char: Character): char is NumberChar => {
  return Object.values(NumberChar).includes(char as NumberChar);
};

export const characterSvgs: Record<Character, number | string> = {
  [Letter.KAGYI]: require("@/assets/chars/kagyi.svg"),
  [Letter.KHAGWE]: require("@/assets/chars/khagwe.svg"),
  [Letter.GANGE]: require("@/assets/chars/gange.svg"),
  [Letter.GHAGYI]: require("@/assets/chars/ghagyi.svg"),
  [Letter.NGA]: require("@/assets/chars/nga.svg"),
  [Letter.SALONE]: require("@/assets/chars/salone.svg"),
  [Letter.SALEIN]: require("@/assets/chars/salein.svg"),
  [Letter.ZAGWE]: require("@/assets/chars/zagwe.svg"),
  [Letter.ZAMYINZWE]: require("@/assets/chars/zamyinzwe.svg"),
  [Letter.NYA]: require("@/assets/chars/nya.svg"),
  [Letter.NNYA]: require("@/assets/chars/nnya.svg"),
  [Letter.TTA]: require("@/assets/chars/tta.svg"),
  [Letter.TTHA]: require("@/assets/chars/ttha.svg"),
  [Letter.DDA]: require("@/assets/chars/dda.svg"),
  [Letter.DDHA]: require("@/assets/chars/ddha.svg"),
  [Letter.NNA]: require("@/assets/chars/nna.svg"),
  [Letter.TA]: require("@/assets/chars/ta.svg"),
  [Letter.THA]: require("@/assets/chars/tha.svg"),
  [Letter.DA]: require("@/assets/chars/da.svg"),
  [Letter.DHA]: require("@/assets/chars/dha.svg"),
  [Letter.NA]: require("@/assets/chars/na.svg"),
  [Letter.PA]: require("@/assets/chars/pa.svg"),
  [Letter.PHA]: require("@/assets/chars/pha.svg"),
  [Letter.BA]: require("@/assets/chars/ba.svg"),
  [Letter.BHA]: require("@/assets/chars/bha.svg"),
  [Letter.MA]: require("@/assets/chars/ma.svg"),
  [Letter.YA]: require("@/assets/chars/ya.svg"),
  [Letter.RA]: require("@/assets/chars/ra.svg"),
  [Letter.LA]: require("@/assets/chars/la.svg"),
  [Letter.WA]: require("@/assets/chars/wa.svg"),
  [Letter.SA]: require("@/assets/chars/sa.svg"),
  [Letter.HA]: require("@/assets/chars/ha.svg"),
  [Letter.LLA]: require("@/assets/chars/lla.svg"),
  [Letter.A]: require("@/assets/chars/a.svg"),
  [NumberChar.ZERO]: require("@/assets/chars/zero.svg"),
  [NumberChar.ONE]: require("@/assets/chars/one.svg"),
  [NumberChar.TWO]: require("@/assets/chars/two.svg"),
  [NumberChar.THREE]: require("@/assets/chars/three.svg"),
  [NumberChar.FOUR]: require("@/assets/chars/four.svg"),
  [NumberChar.FIVE]: require("@/assets/chars/five.svg"),
  [NumberChar.SIX]: require("@/assets/chars/six.svg"),
  [NumberChar.SEVEN]: require("@/assets/chars/seven.svg"),
  [NumberChar.EIGHT]: require("@/assets/chars/eight.svg"),
  [NumberChar.NINE]: require("@/assets/chars/nine.svg"),
};

export const characterPathSvg: Record<Character, number | string> = {
  [Letter.A]: require("@/assets/chars/paths/a.svg"),
  [Letter.KAGYI]: require("@/assets/chars/paths/kagyi.svg"),
  [Letter.KHAGWE]: require("@/assets/chars/paths/khagwe.svg"),
  [Letter.GANGE]: require("@/assets/chars/paths/gange.svg"),
  [Letter.GHAGYI]: require("@/assets/chars/paths/ghagyi.svg"),
  [Letter.NGA]: require("@/assets/chars/paths/nga.svg"),
  [Letter.SALONE]: require("@/assets/chars/paths/salone.svg"),
  [Letter.SALEIN]: require("@/assets/chars/paths/salein.svg"),
  [Letter.ZAGWE]: require("@/assets/chars/paths/zagwe.svg"),
  [Letter.ZAMYINZWE]: require("@/assets/chars/paths/zamyinzwe.svg"),
  [Letter.NYA]: require("@/assets/chars/paths/nya.svg"),
  [Letter.NNYA]: require("@/assets/chars/paths/nnya.svg"),
  [Letter.TTA]: require("@/assets/chars/paths/tta.svg"),
  [Letter.TTHA]: require("@/assets/chars/paths/ttha.svg"),
  [Letter.DDA]: require("@/assets/chars/paths/dda.svg"),
  [Letter.DDHA]: require("@/assets/chars/paths/ddha.svg"),
  [Letter.NNA]: require("@/assets/chars/paths/nna.svg"),
  [Letter.TA]: require("@/assets/chars/paths/ta.svg"),
  [Letter.THA]: require("@/assets/chars/paths/tha.svg"),
  [Letter.DA]: require("@/assets/chars/paths/da.svg"),
  [Letter.DHA]: require("@/assets/chars/paths/dha.svg"),
  [Letter.NA]: require("@/assets/chars/paths/na.svg"),
  [Letter.PA]: require("@/assets/chars/paths/pa.svg"),
  [Letter.PHA]: require("@/assets/chars/paths/pha.svg"),
  [Letter.BA]: require("@/assets/chars/paths/ba.svg"),
  [Letter.BHA]: require("@/assets/chars/paths/bha.svg"),
  [Letter.MA]: require("@/assets/chars/paths/ma.svg"),
  [Letter.YA]: require("@/assets/chars/paths/ya.svg"),
  [Letter.RA]: require("@/assets/chars/paths/ra.svg"),
  [Letter.LA]: require("@/assets/chars/paths/la.svg"),
  [Letter.WA]: require("@/assets/chars/paths/wa.svg"),
  [Letter.SA]: require("@/assets/chars/paths/sa.svg"),
  [Letter.HA]: require("@/assets/chars/paths/ha.svg"),
  [Letter.LLA]: require("@/assets/chars/paths/lla.svg"),
  [NumberChar.ZERO]: require("@/assets/chars/paths/zero.svg"),
  [NumberChar.ONE]: require("@/assets/chars/paths/one.svg"),
  [NumberChar.TWO]: require("@/assets/chars/paths/two.svg"),
  [NumberChar.THREE]: require("@/assets/chars/paths/three.svg"),
  [NumberChar.FOUR]: require("@/assets/chars/paths/four.svg"),
  [NumberChar.FIVE]: require("@/assets/chars/paths/five.svg"),
  [NumberChar.SIX]: require("@/assets/chars/paths/six.svg"),
  [NumberChar.SEVEN]: require("@/assets/chars/paths/seven.svg"),
  [NumberChar.EIGHT]: require("@/assets/chars/paths/eight.svg"),
  [NumberChar.NINE]: require("@/assets/chars/paths/nine.svg"),
};

// 0 <-> 1
// x and y components
export const characterScaling: Record<Character, [number, number]> = {
  [Letter.KAGYI]: [0.8, 0.8],
  [Letter.KHAGWE]: [0.55, 0.55],
  [Letter.GANGE]: [0.5, 0.5],
  [Letter.GHAGYI]: [0.8, 0.8],
  [Letter.NGA]: [0.5, 0.5],
  [Letter.SALONE]: [0.55, 0.55],
  [Letter.SALEIN]: [0.85, 0.85],
  [Letter.ZAGWE]: [0.7, 0.8],
  [Letter.ZAMYINZWE]: [0.7, 0.7],
  [Letter.NYA]: [0.8, 0.8],
  [Letter.NNYA]: [0.8, 0.8],
  [Letter.TTA]: [0.6, 0.6],
  [Letter.TTHA]: [0.55, 0.55],
  [Letter.DDA]: [0.55, 0.55],
  [Letter.DDHA]: [0.55, 0.55],
  [Letter.NNA]: [0.9, 0.9],
  [Letter.TA]: [0.8, 0.8],
  [Letter.THA]: [0.8, 0.8],
  [Letter.DA]: [0.55, 0.55],
  [Letter.DHA]: [0.55, 0.55],
  [Letter.NA]: [0.55, 0.55],
  [Letter.PA]: [0.55, 0.55],
  [Letter.PHA]: [0.55, 0.55],
  [Letter.BA]: [0.55, 0.55],
  [Letter.BHA]: [0.8, 0.8],
  [Letter.MA]: [0.55, 0.55],
  [Letter.YA]: [0.8, 0.8],
  [Letter.RA]: [0.7, 0.7],
  [Letter.LA]: [0.8, 0.8],
  [Letter.WA]: [0.55, 0.55],
  [Letter.SA]: [0.8, 0.8],
  [Letter.HA]: [0.8, 0.8],
  [Letter.LLA]: [0.55, 0.55],
  [Letter.A]: [0.8, 0.8],
  [NumberChar.ZERO]: [0.55, 0.55],
  [NumberChar.ONE]: [0.55, 0.55],
  [NumberChar.TWO]: [0.55, 0.55],
  [NumberChar.THREE]: [0.5, 0.5],
  [NumberChar.FOUR]: [0.5, 0.5],
  [NumberChar.FIVE]: [0.5, 0.5],
  [NumberChar.SIX]: [0.5, 0.5],
  [NumberChar.SEVEN]: [0.5, 0.5],
  [NumberChar.EIGHT]: [0.55, 0.55],
  [NumberChar.NINE]: [0.5, 0.5],
};
